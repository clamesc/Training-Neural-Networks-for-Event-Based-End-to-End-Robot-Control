...
h5f3 = h5py.File(path2 + '/braitenberg_performance_data.h5', 'r')
...
distance3 = np.array(h5f3['distance'], dtype=float)
position3 = np.array(h5f3['position'], dtype=float)
...
plt.axvline(x=x4, linestyle='--', color='0.75')
plt.axvline(x=x5, linestyle='--', color='0.75')
plt.plot(position3,distance3, color='b')
ax5.set_xlim((0,x6))
...
ax6 = plt.subplot(gs[5], sharex=ax2)
ax6.set_xticklabels([])
ax6.set_yticklabels([])
ax6.set_ylim((-lim,lim))
ax6.tick_params(axis='both', which='both', direction='in', bottom=True, top=True, left=True)
ax6.set_title('Braitenberg \ne = '+str('{:4.3f}'.format(s3)), loc='left', size='medium', position=(1.1,0.2))
plt.axhline(y=0, linewidth=0.5, color='0.')
plt.hist(distance3, bins=b, normed=True, color='b', linewidth=2, orientation=u'horizontal')

ax8 = plt.subplot(gs[7], sharex=ax2)
ax8.set_xticklabels([])
ax8.set_yticklabels([])
